		...
		if(isset($_GET['content']))
			...
			elseif(isset($_GET['madm'])) {
					...
					?>
						...
						$conn = mysqli_connect($servername, $username, $password, $database);
						$total_results1 = mysqli_query($conn,"SELECT COUNT(*) as Num FROM sanpham where madm='{$_GET['madm']}'");
						$row = mysqli_fetch_assoc($total_results1);
	...
	?>
                ...
                <div class="sanpham">			
					...
					<div class="sanphamcon">
					    <?php
							$conn = mysqli_connect($servername, $username, $password, $database);
						    $sql1="select * from sanpham inner join danhmuc on sanpham.madm = danhmuc.madm where dequi=1 order by idsp  DESC limit 9 ";
							$result1= mysqli_query($conn, $sql1);
						?>
						<?php 
						  while ($row=mysqli_fetch_array($result1))
